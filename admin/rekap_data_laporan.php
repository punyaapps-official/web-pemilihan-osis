...
<?php 
	...
	<center>
		<a class="hapus-data" href="data_hadir.php">KEMBALI</a> 
		<a class="unduh-data" href="rekap_data_kelas_laporan.php">UNDUH KE EXCEL</a> 
	</center>	
    ...
    function jumlah_peserta($nama_kelas, $db) {
        $data = mysqli_query($db, "SELECT * FROM tb_pemilih WHERE nama_kelas = '$nama_kelas'");
        $jlh_peserta = mysqli_num_rows($data);
        return $jlh_peserta; 
    }

    function jumlah_hadir($nama_kelas, $db) {
        $data = mysqli_query($db, "SELECT * FROM tb_pilih WHERE nama_kelas = '$nama_kelas'");
        $jlh_hadir = mysqli_num_rows($data);
        return $jlh_hadir;
    }
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
                <?= jumlah_peserta($nama_kelas[$i-1], $db) ?>
            </td>
            <td>
                <?= jumlah_hadir($nama_kelas[$i-1], $db) ?>
            </td>
        </tr>
